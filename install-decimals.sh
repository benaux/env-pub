...
rm -f $redir/security
ln -s $security $redir/security
rm -f $redir/all
ln -s $security $redir/all

die () { echo $@; exit 1; }
log () { echo $@;  }

[ -d "$usrnodes" ] || die "Err: no usernodes"
[ -d "$usrfiles" ] || die "Err: no usrfiles"
...
link_decimal () {
  ...
  case "$pdec" in 
    6[1-9])
      link_to_place $bd--$pname $dir $security
      link_to_r $bd--$pname $dir $redir
      ;;
    [0-9]*)
